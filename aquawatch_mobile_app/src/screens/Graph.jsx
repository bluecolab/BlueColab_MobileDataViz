...
function Graph() {
  ...
  const renderItem = useCallback(
    ({ item }) => {
      if (!defaultTempUnit && !defaultLocation) return <Text>Loading...</Text>;
      return (
        <DataGraph
          loading={loading}
  ...
  return (
    <View className="bg-defaultbackground pb-[100]  dark:bg-defaultdarkbackground">
      <RenderTab />
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 175 }}>
        ...
        <View className="my-default flex-row justify-center">
          {parameterInfo.map((_, index) => (
            <Text
              key={index}
              className={`mx-1 h-2.5 w-2.5 rounded-full ${currentIndex === index ? 'bg-blue-500' : 'bg-gray-400'}`}
            />
          ))}
        </View>

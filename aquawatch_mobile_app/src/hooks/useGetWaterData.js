  ...
  const fetchData = (
    ...
    switch (defaultLocation) {
      ...
      case 'Bronx River':
        ...
        query = isCurrentData
          ? `?sites=${stationIds[defaultLocation] ?? '01376269'}&period=P2D&format=json`
          : `?sites=${stationIds[defaultLocation] ?? '01376269'}&startDT=${year}-${month}-${start_day}&endDT=${year}-${month}-${end_day}&format=json`;
        break;
      default:
        baseURL = 'https://colabprod01.pace.edu/api/influx/sensordata/Ada/';
        query = isCurrentData

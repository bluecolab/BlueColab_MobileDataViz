  ...
  return (
    <View>
      <Text className="text-md py-4 text-center text-white">
        As of{' '}
        {minutes !== null
          ? `${Math.floor(minutes / 60)} minute(s) ago`
          : 'Loading...'}
      </Text>
    </View>
  );
...
export default function QuickCurrentData({ handleMiddlePress }) {
  ...
  const ParamView = ({ param, name, unit }) => (
    <View
      style={{ width: itemWidth }}
      className="flex items-center justify-center rounded-lg ">
      <Text className="text-center  text-2xl text-white">
        {param} {unit}
        {name === 'WQI' ? <Text className="text-base">/100</Text> : ''}
      </Text>
        ...
        <LinearGradient
          ...
          <View className="flex flex-row flex-wrap items-center justify-center gap-4 pt-4">
            ...
            {defaultLocation === 'Choate Pond' ? (
              <ParamView
                param={!isNaN(wqi) ? wqi?.toFixed(2) : 'NA'}
                name="WQI"
              />
            ) : (
              <></>
